...
public class EmployeeDAO {
    ...
    public EmployeeDAO(Context context) {
        ...
        db = helper.getWritableDatabase(); // thêm chức năng thêm xóa sửa

    }
    @SuppressLint("Range")
    public List<Employee> get(String sql, String ...selectArgs){
        List<Employee> list = new ArrayList<>();
        Cursor cursor = db.rawQuery(sql, selectArgs); //truy vấn
        while (cursor.moveToNext()){
            ...
            list.add(emp); //đọc danh sách
        }
        return list; //trả về danh sách đạt được
    }
    // Hàm getAll
